...
<body topmargin="0" rightmargin="0" bottommargin="0" leftmargin="0" marginwidth="0" marginheight="0" width="100%" style="border-collapse: collapse; border-spacing: 0; margin: 0; padding: 0; width: 100%; height: 100%; -webkit-font-smoothing: antialiased; text-size-adjust: 100%; -ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%; line-height: 100%;
	...
	max-width: 560px;" class="container">
			...
			font-family: sans-serif; text-align: justify; word-break:break-all" class="paragraph">
                            Hola, {{$elector->nombre}}, un administrador de iElect ha editado tus datos personales. Tus nuevos datos son los siguientes: 
                            <br>
                            <br>
                            <b>Nombre completo: </b> {{ $elector->nombre." ".$elector->apellido_p." ".$elector->apellido_m}}
                            <br>
                            <b>Domicilio: </b> {{ $elector->calle." ".$elector->ext_num." ".$elector->int_num.', '.$elector->colonia." ".$elector->cp}}
                            <br>
                            <b>Correo electrónico: </b> {{ $elector->email }}
                            <br>
                            <b>Edad: </b> {{ \Carbon\Carbon::parse($elector->fecha_nac)->diff(\Carbon\Carbon::now())->format('%y años')}}
                            <br>
                            <b>Sexo: </b> 
                                @if ($elector->sexo == 'h')
                                    Hombre
                                ...
                                @endif
                            <br>
                            <b>Ocupación: </b> {{ $elector->job->nombre}}
                            <br>
                            <b>Teléfono: </b> {{ $elector->telefono}}
                            <br>
                            <b>Clave de elector: </b> {{ $elector->clave_elector}}
                            <br>
                            <b>Facebook: </b> {{ $elector->facebook}}
                            <br>
                            <b>Twitter: </b> {{ $elector->twitter}}
                            <br>
                            @if ($bfotoa == 1 || $bfotor == 1 || $bfotoe == 1 || $bfotod == 1)
                                <b>Se han eliminado las fotos:</b>
                                <ul>
                                    @if ($bfotoa == 1)
                                    <li>
                                        Foto de credencial anverso
                                    </li>
                                    @endif
                                    @if ($bfotor == 1)
                                    <li>
                                        Foto de credencial reverso
                                    </li>
                                    @endif
                                    @if ($bfotoe == 1)
                                    <li>
                            ...
                            @endif
                            @if ($bfotoa == 2 || $bfotor == 2 || $bfotoe == 2 || $bfotod == 2)
                                <b>Se han cambiado las fotos:</b>
                                <ul>
                                    @if ($bfotoa == 2)
                                    <li>
                                        Foto de credencial anverso
			...
			font-family: sans-serif; text-align: justify;" class="paragraph">
                            ...
                            Si no estás de acuerdo con este cambio puedes ponerte en contacto con el correo electrónico proporcionado en los <a href="{{ route('terminoscondiciones') }}" target="_blank">Términos y Condiciones</a> y el <a href="{{ route('avisoprivacidad') }}" target="_blank">Aviso de Privacidad</a> en cualquier momento, o puedes solicitar la eliminación o edición de los datos personales que proporcionaste.
